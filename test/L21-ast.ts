...
//Assignment2//
export const makeForExp = (index: VarDecl, start: NumExp, end: NumExp, body: CExp): ForExp =>
    ({tag: "ForExp", var: index, start: start, end: end, body: body});
...
const parseForExp = (variable: Sexp, params: Sexp[]): Result<ForExp> =>
    params.length !== 3 || ! isIdentifier(variable)  ? makeFailure("Expression not of the form (for <Vardecl> <cexp> <cexp> <cexp>)"):
            bind(mapResult(parseL21CExp, params),
                (cexp: CExp[]) => isNumExp(cexp[0]) && isNumExp(cexp[1]) && cexp[0].val <= cexp[1].val ? makeOk(makeForExp(makeVarDecl(variable), cexp[0], cexp[1], cexp[2])) :
        makeFailure("Expression not of the form (for <VarDecl> <NumExp> <NumExp> <cexp>+) or second NumExp is bigger than first NumExp"));
